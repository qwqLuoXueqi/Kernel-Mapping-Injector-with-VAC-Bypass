...
namespace ManualMapInjector {
    ...
    struct ShellcodeData {
        ...
        DWORD ImportDirRVA;
        DWORD ImportDirSize;
        DWORD TlsDirRVA;            // TLS 目录 RVA: 加壳(VMP)DLL 的壳 runtime 初始化靠它
    };
    ...
    inline DWORD WINAPI Shellcode(LPVOID lpParameter) {
        ...
        // TLS 回调执行: 加壳 DLL(VMP) 的壳 runtime 初始化在 TLS 回调里,
        // 必须在 DllMain 之前逐个跑, 否则 DllMain(壳入口) 在未初始化 runtime 上必崩。
        // AddressOfCallBacks 是 VA(经重定位后已指向映射内正确地址), 数组以 NULL 结尾。
        if (data->TlsDirRVA != 0 && data->TlsDirRVA < imageSize) {
            PIMAGE_TLS_DIRECTORY tls = (PIMAGE_TLS_DIRECTORY)((LPBYTE)imageBase + data->TlsDirRVA);
            if (tls->AddressOfCallBacks) {
                PIMAGE_TLS_CALLBACK* cb = (PIMAGE_TLS_CALLBACK*)(ULONG_PTR)tls->AddressOfCallBacks;
                LPBYTE cbAddr = (LPBYTE)cb;
                if (cbAddr >= (LPBYTE)imageBase && cbAddr < (LPBYTE)imageBase + imageSize) {
                    int ran = 0;
                    for (; *cb && ran < 64; ++cb, ++ran) {   // 上限 64: 防坏数据跑飞
                        __try {
